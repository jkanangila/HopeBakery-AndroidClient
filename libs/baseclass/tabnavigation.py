...
class NavigationItem(ThemableBehavior, ButtonBehavior, BoxLayout):
    ...
    button_width = NumericProperty(dp(40))
    button_height =  NumericProperty(dp(120))
    text = StringProperty()
    icon = StringProperty()
    ...
    def on_release(self):
        # shrink all other buttons within the parent widget
    ...
    def _button_shrink(self):
        ...
        but_anim = Animation(
            ...
            t="linear",
            duration=self.duration,
            icon_color=app.custom_normal_color
        )
        but_anim.start(self)
...
class NavigationBar(ThemableBehavior, BoxLayout):
    ...
    def _update(self, *args):
        ...
        space = self.height - button_sizes
        spacing = space / (len(buttons) + 1)
        self.ids._button_box.spacing = spacing
        self.ids._button_box.padding = [0, dp(spacing), 0, dp(spacing + 80)]
